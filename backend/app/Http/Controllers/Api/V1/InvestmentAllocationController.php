...
class InvestmentAllocationController extends Controller
{
    ...
    {
        return new InvestmentAllocationResource($service->allocate($investment, \App\Models\TransportJob::findOrFail($request->integer('transport_job_id')), (float) $request->input('amount'), $request->input('notes')));
    }
