...
class InvestorPayoutService
{
    public function calculateDistribution(TransportJob $job, Investment $investment, ?string $notes = null): InvestorProfitDistribution
    {
        return DB::transaction(function () use ($job, $investment, $notes) {
            ...
            if ($job->financially_locked_at) $this->reject('job', 'This job is financially locked.');
            if (InvestorProfitDistribution::query()->where('investment_id', $investment->id)->where('transport_job_id', $job->id)->exists()) $this->reject('distribution', 'A distribution for this investment and job already exists.');

            $allocation = $investment->allocations()->where('transport_job_id', $job->id)->where('status', AllocationStatus::Active)->first();
            if (! $allocation) $this->reject('allocation', 'This investment has no active allocation on the job.');

            $basis = round((float) $job->final_profit, 2);
            $value = (float) $investment->return_rate;
            $amount = $investment->return_type === InvestmentReturnType::ProfitShare
                ? round($basis * ($value / 100), 2)
                : round($value, 2);
            ...
            if ($fundingInvestments->isNotEmpty() && $calculated === $fundingInvestments->count()) $job->update(['financially_locked_at' => now()]);
