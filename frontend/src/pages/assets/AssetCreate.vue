...
const submit = async () => {
  ...
  try {
    ...
    const asset = await assetStore.createAsset(payload)

    showToast(`${form.name || 'Vehicle'} added`)
    ...
    if (error.response?.status === 422) {
      ...
      generalError.value = error.response?.data?.message || 'Unable to create vehicle.'
    }
  } finally {
  ...
  <div class="asset-create-page">
    ...
    <div class="page-head">
      <div>
        ...
        <h1>Add vehicle</h1>
        <p class="page-sub">Register a company vehicle — identification, value, and status.</p>
      </div>
    </div>

    <form class="asset-form-layout" @submit.prevent="submit">
      <!-- ===== Main column — MUST wrap the fragment component ===== -->
      <div class="form-main">
        <AssetFormFields
          :form="form"
      ...
      <aside class="form-aside">
        <div class="card panel-card">
          <h2 class="panel-title">
            Preview
            <InfoTip label="Updates live as you fill in the form." />
          </h2>
